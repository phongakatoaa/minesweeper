...
int main() {
	...
	for (;;) {
		...
		if (strcmp(flag, "f")==0||strcmp(flag, "F")==0) {
			flag_placing(m,n);
			...
			if (mines_check(m, n) == 1) {
				...
				//player_dont_hit_mines(m,n);
					...
					if (surround_mines(m, n) == 0) { expand(m, n); } //display_bugged_play_board();
					else { player_dont_hit_mines(m, n); }
					if (mines_check(m, n) == 0) { block_count++; }
					display_play_board();
				}
			}
		}
